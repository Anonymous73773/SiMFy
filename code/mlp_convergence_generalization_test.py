...
def train(dataset):
    ...
    dt_string = dataset
    main_dirName = os.path.join("TEST_RE/mlp_conver_general_models/" + dt_string)
    ...
    for epoch in range(max_epochs):
        ...
        file_training.write("******\nEpoch {:04d} | Loss {:.4f}| time {:.4f} =========================================".
                            format(epoch, loss_epoch / _batch, epoch_time - time_begin) + '\n')
        torch.save(model, model_path + '/' + dataset + f"_{epoch}.pth")

        mrr_train, hit1_train, hit3_train, hit10_train = test(dataset, epoch, train_data)
        mrr_test, hit1_test, hit3_test, hit10_test = test(dataset, epoch, test_data)

        file_training.write("\n######### train_set #########\n")
        file_training.write(f"MRR: {mrr_train}" + '\n')
        file_training.write(f"Hits@1: {hit1_train}" + '\n')
        file_training.write(f"Hits@3: {hit3_train}" + '\n')
        file_training.write(f"Hits@10: {hit10_train}" + '\n')

        file_training.write("\n######### test_set #########\n")
        file_training.write(f"MRR: {mrr_test}" + '\n')
        file_training.write(f"Hits@1: {hit1_test}" + '\n')
        file_training.write(f"Hits@3: {hit3_test}" + '\n')
        file_training.write(f"Hits@10: {hit10_test}" + '\n')

        epoch_gap = mrr_train - mrr_test
        print(f"epoch_gap: {epoch_gap}")
        file_training.write(f"\nepoch_gap: {epoch_gap}" + '\n')
...
def test(dataset, epoch, test_data, mode="test"):

    alpha = 0.001  # balance freq and mlp
    ...
    model = torch.load(model_path + '/' + dataset + f"_{epoch}.pth")
    ...
    with torch.no_grad():
        ...
        for _, data in tqdm(enumerate(test_loader, 0)):
            ...
            freq_outputs = get_outputs(
                ...
                t_list.cpu().detach().numpy().tolist(), num_rels, k, True)
